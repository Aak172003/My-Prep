...
void merge(int *arr, int start, int end) {
    ...
    int *first = new int[len1];            // Left half size ka Dynamically array bnana hai 
    int *second = new int[len2];           // Right half size ka Dynamically array bnana hai 
    ...
    // yaha humne 2 new dynamically array ko create krte hai , kyuki upr wale array ko via mergesort 
...
void mergeSort(int *arr, int start, int end) {
    ...
    //base case 
    // agr ek hi element hai , to s and e both point on same value , so it is sorted 
    if(start >= end) {
        return;
    }

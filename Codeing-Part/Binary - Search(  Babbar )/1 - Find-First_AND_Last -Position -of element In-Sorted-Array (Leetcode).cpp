...
public:
    void lowerBound(vector<int>&nums,int target,vector<int>&ans){
        ...
        while(left<=right)
        {
            ...
            // ya to if execute hoga , ya to else if execute hoga , ya to else execute hoga 
            if(nums[mid]==target)
            {
                ans[0]=mid;
    ...
    void higherBound(vector<int>&nums,int target,vector<int>&ans)
    {
        ...
        while(left<=right)
        {
            ...
            //  which means jo mid nikal kr aaya hai  wo chota hai or target or bhi bda hai iski wjh se hume right me search krna pdega 
            else if(nums[mid]<target)
            {
                left=mid+1;
            }
            // means mid jo nikal kr aaya hai wo bda hai or target chota hai , which means hum left me search krna hai 
            else right=mid-1;
        }
    }
